...
function enemy:burrow()
  ...
  local effect = map:create_custom_entity{
    direction = 0, x=x, y=y+2, layer = z, width = 16, height = 16, sprite = "enemies/misc/splash"
  }
  effect:get_sprite():set_animation("splash", function() effect:remove() end)
  if close_to_hero() then sol.audio.play_sound"splash" end
